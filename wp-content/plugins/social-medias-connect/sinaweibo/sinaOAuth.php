...
$APP_KEY='3033277072';
$APP_SECRET='1751a3781168871b55f33a6e2ec5e79d';
...
/*2011.04.12*/
function smc_sina_verify_credentials(){
	global $APP_KEY, $APP_SECRET;
...
function smc_sina_getAccessToken($oauth_verifier,$oauth_token,$token_secret){
	global $APP_KEY, $APP_SECRET;
	$to=new WeiboOAuth($APP_KEY, $APP_SECRET, $oauth_token, $token_secret);
	return $to->getAccessToken($oauth_verifier);
}
function smc_sina_weibo_update($data,$thumb,$tok){
	global $APP_KEY, $APP_SECRET;
	...
	if($thumb){
		...
		if($resp['error_code']){
			return smc_sina_weibo_update($content,'',$tok);
		}
	}else{
...
function smc_sina_weibo_repost($p,$smcdata){//9105332181
	global $APP_KEY, $APP_SECRET, $wp_smiliessearch;
	$p['comment']=preg_replace_callback($wp_smiliessearch, 'smc_sina_convert_smilies', $p['comment']);
	$content=get_comment_str_length($p);
	if(!$p['weibosync']){
		return smc_sina_weibo_update($content,'',array('oauth_token'=>$smcdata['oauth_access_token'],'oauth_token_secret'=>$smcdata['oauth_access_token_secret']));
	}
...
function smc_sina_convert_smilies($smiley){
	if (count($smiley) == 0) {
		return '';
	}
	$smiley_array=array(
...
function smc_sina_weibo_timeline($r,$tok){
	global $APP_KEY, $APP_SECRET;
	$to=new WeiboOAuth($APP_KEY, $APP_SECRET, $tok['oauth_token'], $tok['oauth_token_secret']);
	switch($r['type']){
...
function _smc_sina_make_at_user($matches){
	if(strpos($matches[1],'#')!==false){
		return $matches[0];
	}else return '<a href="http://weibo.com/n/'.$matches[1].'" rel="nofollow">'.$matches[0].'</a>';
}
function _smc_sina_make_topic($matches){
