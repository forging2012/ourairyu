/***************
 * Created on March 13, 2012 by Ourai Lin(http://ourai.us/)
 * 
 * 
 * 
		...
		if ( items && $.isArray(items) ) {
			$.each( items, function( idx, item ) {
				if ( type == "article" && !internal.articles.items[ "article-" + item.ID ] ) {
					...
					html.push( "<li id=\"article-" + item.ID + "\" class=\"" + cats.join(" ") + "\"><a class=\"clr\" href=\"" + item.permalink + "\" title=\"阅读《" + item.postTitle + "》\"><h3 class=\"title\">" + item.postTitle + "<sup title=\"在新窗口中阅读《" + item.postTitle + "》\">在新窗口中阅读《" + item.postTitle + "》</sup></h3><span class=\"date\">" + item.postDate + "</span></a></li>" );	// <span class=\"comments\" title=\"被吐槽 " + item.commentCount + " 次\">" + item.commentCount + "</span>
				}
