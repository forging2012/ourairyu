...
$APP_KEY='fda955fa85fe4196afb583731958b064';
$APP_SECRET='a9c6cdc7fbebb7cceff563759ee01617';
...
/* 2011.04.12 */
function smc_qq_verify_credentials(){
	global $APP_KEY, $APP_SECRET;
...
function smc_qq_getAccessToken($oauth_verifier,$oauth_token,$token_secret){
	global $APP_KEY, $APP_SECRET;
	$to=new MBOpenTOAuth($APP_KEY, $APP_SECRET, $oauth_token, $token_secret);
	return $to->getAccessToken($oauth_verifier);
}
function smc_qq_weibo_update($data,$thumb,$tok){
	global $APP_KEY, $APP_SECRET;
	...
	if($thumb){
		...
		if($resp->ret){
			return smc_qq_weibo_update($content,'',$tok);
		}
	}else{
...
function smc_qq_weibo_repost($p,$smcdata){
	global $APP_KEY, $APP_SECRET, $wp_smiliessearch;
	$p['comment']=preg_replace_callback($wp_smiliessearch, 'smc_qq_convert_smilies', $p['comment']);
	$content=get_comment_str_length($p);
	if(!$p['weibosync']){
		return smc_qq_weibo_update($content,'',array('oauth_token'=>$smcdata['oauth_access_token'],'oauth_token_secret'=>$smcdata['oauth_access_token_secret']));
	}
...
function smc_qq_convert_smilies($smiley){
	if (count($smiley) == 0) {
		return '';
	}
	$smiley_array=array(
...
function smc_qq_weibo_timeline($r,$tok){
	global $APP_KEY, $APP_SECRET;
	$to=new MBOpenTOAuth($APP_KEY, $APP_SECRET, $tok['oauth_token'], $tok['oauth_token_secret']);
	switch($r['type']){
...
function _smc_qq_make_at_user($matches){
	if(strpos($matches[1],'#')!==false){
		return $matches[0];
	}else return '<a href="http://t.qq.com/'.$matches[1].'" rel="nofollow">'.$matches[0].'</a>';
}
function _smc_qq_make_topic($matches){
