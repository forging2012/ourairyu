...
$APP_KEY='zaQudweygpqNJUQ2';
$APP_SECRET='GbCxdyHoscZn5TFuoBHrJ1W0CUc8yY72';
...
/* 2011.04.12 */
function smc_163_verify_credentials(){
...
function smc_163_getAccessToken($oauth_verifier,$oauth_token,$token_secret){
	global $APP_KEY, $APP_SECRET;
	$to=new Weibo163OAuth($APP_KEY, $APP_SECRET, $oauth_token, $token_secret);
	return $to->getAccessToken($oauth_verifier);
}
function smc_163_weibo_update($data,$thumb,$tok){//-8737476567477287845
	global $APP_KEY, $APP_SECRET;
	...
	if($thumb){
		...
		if($pic['error_code']){
			return smc_163_weibo_update($content,'',$tok);
		}
...
function smc_163_weibo_repost($p,$smcdata){//-8737476567477287845
	global $APP_KEY, $APP_SECRET;
	$content=get_comment_str_length($p,140,true);
	if(!$p['weibosync']){
		return smc_163_weibo_update($content,'',array('oauth_token'=>$smcdata['oauth_access_token'],'oauth_token_secret'=>$smcdata['oauth_access_token_secret']));
	}
...
function smc_163_weibo_timeline($r,$tok){
	global $APP_KEY, $APP_SECRET;
	$to=new Weibo163OAuth($APP_KEY, $APP_SECRET, $tok['oauth_token'], $tok['oauth_token_secret']);
	switch($r['type']){
